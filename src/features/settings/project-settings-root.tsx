    ...
    <div className="space-y-4">
      ...
      <section className="rounded-lg border border-zinc-200/70 bg-white/70 p-4 shadow-soft dark:border-zinc-700/60 dark:bg-zinc-900/70">
        ...
        <div className="grid grid-cols-1 gap-3 md:grid-cols-2">
          <label className="text-sm">
            <div className="mb-1 text-zinc-600 dark:text-zinc-400">{t("settings.project.name")}</div>
            <input
          ...
          <label className="text-sm md:col-span-2">
            <div className="mb-1 text-zinc-600 dark:text-zinc-400">{t("settings.project.description")}</div>
            <textarea
              className="w-full rounded border border-zinc-300 bg-white px-3 py-2 text-sm dark:border-zinc-700 dark:bg-zinc-800"
              rows={3}
              value={description}
      ...
      <section className="rounded-lg border border-zinc-200/70 bg-white/70 p-4 shadow-soft dark:border-zinc-700/60 dark:bg-zinc-900/70">
        ...
        <div className="mb-2 flex flex-wrap items-center gap-2 text-sm">
          <select className="rounded border border-zinc-300 bg-white px-2 py-1 dark:border-zinc-700 dark:bg-zinc-900" value={currentProjectId} onChange={async (e)=>{
        ...
        <div className="flex flex-wrap items-center gap-2 text-sm">
          <button type="button" className="rounded bg-zinc-100 px-2 py-1 dark:bg-zinc-800" onClick={async ()=>{
            if (!adapter) return
            const p = await adapter.createProject("営業管理プロジェクト")
            await adapter.createColumns(p.id, ["問い合わせ","商談","見積もり","受注"]) 
            setProjects(await adapter.listProjects())
          }}>営業管理テンプレート</button>
          <button type="button" className="rounded bg-zinc-100 px-2 py-1 dark:bg-zinc-800" onClick={async ()=>{
            const p = await adapter.createProject("顧客対応プロジェクト")
            await adapter.createColumns(p.id, ["未対応","対応中","対応済み","保留"]) 
            setProjects(await adapter.listProjects())
          }}>顧客対応テンプレート</button>
          <button type="button" className="rounded bg-zinc-100 px-2 py-1 dark:bg-zinc-800" onClick={async ()=>{
            const p = await adapter.createProject("中途採用プロジェクト")
            await adapter.createColumns(p.id, ["書類選考","一次面談","最終面談","採用"]) 
            setProjects(await adapter.listProjects())
          }}>採用管理テンプレート</button>
        </div>
        <div className="mt-2 text-xs text-zinc-500">テンプレートを作成後、上のセレクトで切り替えると全画面の対象プロジェクトが変更されます。</div>
      </section>
      ...
      <section className="rounded-lg border border-zinc-200/70 bg-white/70 p-4 shadow-soft dark:border-zinc-700/60 dark:bg-zinc-900/70">
        ...
        <div className="mt-2 text-xs text-zinc-500">削除・補充は現在選択中のプロジェクトに対して実行されます。</div>
      </section>
      <section className="rounded-lg border border-zinc-200/70 bg-white/70 p-4 shadow-soft dark:border-zinc-700/60 dark:bg-zinc-900/70">
        ...
        {!FF_TASK_BACKEND && (
          <div className="mt-2 text-xs text-zinc-500">{t("settings.dataSource.httpDisabled")}</div>
        )}
      </section>
      <section className="rounded-lg border border-zinc-200/70 bg-white/70 p-4 shadow-soft dark:border-zinc-700/60 dark:bg-zinc-900/70">
        <h2 className="mb-2 text-sm font-medium">{t("settings.members")}</h2>
        <div className="mb-3 flex flex-wrap items-center gap-2 text-sm">
          <input className="w-48 rounded border border-zinc-300 bg-white px-3 py-1.5 text-sm dark:border-zinc-700 dark:bg-zinc-800" placeholder="氏名" value={newMemberName} onChange={(e)=>setNewMemberName(e.target.value)} />
          <input className="w-64 rounded border border-zinc-300 bg-white px-3 py-1.5 text-sm dark:border-zinc-700 dark:bg-zinc-800" placeholder="メールアドレス" value={newMemberEmail} onChange={(e)=>setNewMemberEmail(e.target.value)} />
          <select className="rounded border border-zinc-300 bg-white px-2 py-1 dark:border-zinc-700 dark:bg-zinc-900" value={newMemberRole} onChange={(e)=>setNewMemberRole(e.target.value as Role)}>
            <option value="admin">admin</option>
            <option value="editor">editor</option>
            <option value="viewer">viewer</option>
          </select>
          <button type="button" className="rounded bg-brand-600 px-3 py-1.5 text-sm text-white" onClick={async ()=>{
            if (!project || !newMemberEmail.trim() || !newMemberName.trim()) return
            const u = await adapter.addUser(newMemberEmail.trim(), newMemberName.trim())
            const m = await adapter.addProjectMember(project.id, u.id, newMemberRole)
        ...
        <ul className="divide-y divide-zinc-200/70 text-sm dark:divide-zinc-700/60">
          {members.map((m)=>(
            <li key={m.id} className="flex items-center justify-between py-2">
              <div>
                <div className="font-medium">{usersById[m.userId]?.displayName ?? m.userId}</div>
                <div className="text-xs text-zinc-500">{usersById[m.userId]?.email}</div>
              </div>
              <div className="flex items-center gap-2">
                <select className="rounded border border-zinc-300 bg-white px-2 py-1 dark:border-zinc-700 dark:bg-zinc-900" value={m.role} onChange={async (e)=>{
                  // ローカルアダプタは role 更新API未実装のため、remove->add で代替
                  await adapter.removeProjectMember(m.id)
                  const nm = await adapter.addProjectMember(project!.id, m.userId, e.target.value as Role)
                  setMembers((prev)=>prev.map((x)=> x.id===m.id ? nm : x))
                }}>
                  <option value="admin">admin</option>
                  <option value="editor">editor</option>
                  <option value="viewer">viewer</option>
      ...
      <section className="rounded-lg border border-zinc-200/70 bg-white/70 p-4 shadow-soft dark:border-zinc-700/60 dark:bg-zinc-900/70">
        ...
        <p className="mb-2 text-xs text-zinc-500">ブラウザにのみ保存されます。公開環境ではサーバープロキシの利用を推奨します。</p>
        <div className="flex flex-wrap items-center gap-2">
          <input
            ...
            onBlur={() => updateSettings({ geminiApiKey: apiKey.trim() || undefined })}
          />
          <button
            type="button"
          ...
          <button
            type="button"
            className="rounded bg-zinc-100 px-3 py-1.5 text-sm dark:bg-zinc-800"
            onClick={() => { clearSetting("geminiApiKey"); setApiKey("") }}
          >
            クリア
          </button>
        </div>
        <div className="mt-2 text-xs text-zinc-500">分解ページのAI呼び出しで使用します。未設定時はローカル分解にフォールバックします。</div>
      </section>

      <section className="rounded-lg border border-zinc-200/70 bg-white/70 p-4 shadow-soft dark:border-zinc-700/60 dark:bg-zinc-900/70">
        <h2 className="mb-2 text-sm font-medium">{t("settings.ai")}</h2>
        <div className="flex items-center gap-2 text-sm">
          <label className="text-zinc-600 dark:text-zinc-400" htmlFor="ai-model">{t("settings.ai.model")}:</label>
          <select
            id="ai-model"
            className="rounded border border-zinc-300 bg-white px-2 py-1 dark:border-zinc-700 dark:bg-zinc-900"
            value={model || "gemini-2.5-flash"}
            onChange={(e) => { setModel(e.target.value); updateSettings({ geminiModel: e.target.value }) }}
          >
            <option value="gemini-2.5-flash">{t("settings.ai.model.flash")}</option>
