import type { Column, Id, Project, Task, ChecklistItem, Comment, Tag, TaskDependency, User, Member, Role } from "@/types/domain"
...
function seed(): Db {
  ...
  const db: Db = {
    projects: [
      { id: projectId, name: "Secretary", description: "Jootoのようなタスク管理アプリの開発中", createdAt: now(), createdBy: "u1" },
    ],
    boards: [{ id: boardId, projectId, name: "Default", sortIndex: 1000 }],
    columns,
    tasks,
    checklist: [
      { id: "cl1", taskId: "t1", title: "要件を洗い出す", done: false, sortIndex: 1000, dueDate: toISOPlusDays(2) },
    ],
    comments: [
      { id: "cm1", taskId: "t1", authorId: "u1", body: "まずはMVPを固めましょう", createdAt: now() },
    ],
    tags: [
    ...
    dependencies: [],
    users: [
      { id: "u1", email: "user1@example.com", displayName: "ユーザー1", createdAt: now() },
      { id: "u2", email: "user2@example.com", displayName: "ユーザー2", createdAt: now() },
    ],
    members: [
...
export function createLocalAdapter(): DataAdapter {
  return {
    async getDefaultProject(): Promise<Project> {
      ...
      const project = pid ? (db.projects.find((x)=>x.id===pid) ?? db.projects[0]) : db.projects[0]
    ...
    async updateProject(projectId: Id, patch: Partial<Pick<Project, "name" | "description">>): Promise<Project> {
      const db = load()
    ...
    async listTasks(q: ListTasksQuery): Promise<Task[]> {
      ...
      return db.tasks.filter(
        (t) => t.projectId === q.projectId && (!q.columnId || t.columnId === q.columnId)
      )
    },
    ...
    async updateTask(taskId: Id, patch: Partial<Pick<Task, "title" | "description" | "startDate" | "dueDate" | "progress" | "columnId" | "sortIndex" | "parentId" | "estimatedHours">>): Promise<Task | null> {
      const db = load()
      ...
      if (typeof patch.progress === "number") t.progress = Math.max(0, Math.min(100, patch.progress))
      ...
      if (typeof patch.estimatedHours === "number") t.estimatedHours = patch.estimatedHours
      save(db)
      return t
    },
    ...
    async listChecklist(taskId: Id): Promise<ChecklistItem[]> {
      const db = load()
      return db.checklist.filter((c) => c.taskId === taskId).sort((a, b) => a.sortIndex - b.sortIndex)
    },
    async addChecklist(taskId: Id, title: string): Promise<ChecklistItem> {
      ...
      const item: ChecklistItem = { id: nextId("cl"), taskId, title, done: false, sortIndex: max + 1000 }
      db.checklist.push(item)
      save(db)
      return item
    },
    async updateChecklist(itemId: Id, patch: Partial<Pick<ChecklistItem, "title" | "done" | "sortIndex" | "dueDate" | "assigneeId">>): Promise<ChecklistItem | null> {
      const db = load()
      ...
      if (typeof patch.assigneeId !== "undefined") it.assigneeId = patch.assigneeId
      save(db)
      return it
    },
    async toggleChecklist(itemId: Id, done: boolean): Promise<ChecklistItem | null> {
      const db = load()
      const it = db.checklist.find((c) => c.id === itemId)
      if (!it) return null
      it.done = done
    ...
    async addComment(taskId: Id, body: string): Promise<Comment> {
      const db = load()
      const cm: Comment = { id: nextId("cm"), taskId, authorId: "u1", body, createdAt: now() }
      db.comments.push(cm)
      save(db)
      return cm
    },
    ...
    async listProjectTags(projectId: Id): Promise<Tag[]> {
      const db = load()
      return db.tags.filter((t) => t.projectId === projectId).sort((a, b) => a.name.localeCompare(b.name))
    },
    async addProjectTag(projectId: Id, name: string, color: string): Promise<Tag> {
      const db = load()
      const existed = db.tags.find((t) => t.projectId === projectId && t.name === name)
    ...
    async listTaskTags(taskId: Id): Promise<Tag[]> {
      const db = load()
      const ids = db.taskTags.filter((tt) => tt.taskId === taskId).map((x) => x.tagId)
    ...
    async removeTagFromTask(taskId: Id, tagId: Id): Promise<void> {
      const db = load()
      db.taskTags = db.taskTags.filter((x) => !(x.taskId === taskId && x.tagId === tagId))
      save(db)
    },
    ...
    async listTaskAssignees(taskId: Id): Promise<Id[]> {
      const db = load()
      return db.taskAssignees.filter((a) => a.taskId === taskId).map((a) => a.userId)
    },
    async assignTask(taskId: Id, userId: Id): Promise<void> {
      const db = load()
      if (!db.taskAssignees.find((a) => a.taskId === taskId && a.userId === userId)) {
    ...
    async unassignTask(taskId: Id, userId: Id): Promise<void> {
      const db = load()
      db.taskAssignees = db.taskAssignees.filter((a) => !(a.taskId === taskId && a.userId === userId))
      save(db)
    },
    ...
    async addProjectMember(projectId: Id, userId: Id, role: Role): Promise<Member> {
      const db = load()
      const existed = db.members.find((m) => m.projectId === projectId && m.userId === userId)
      if (existed) return existed
      const m: Member = { id: nextId("m"), projectId, userId, role, joinedAt: now() }
      db.members.push(m)
      save(db)
      return m
    },
    async removeProjectMember(memberId: Id): Promise<void> {
      ...
      if (mem) {
        ...
        db.taskAssignees = db.taskAssignees.filter((a) => a.userId !== mem.userId)
        save(db)
      }
    },
    ...
    async createProject(name: string, description?: string): Promise<Project> {
      const db = load()
      const p: Project = { id: nextId("p"), name, description, createdAt: now(), createdBy: "u1" }
      db.projects.push(p)
      // create a board
      db.boards.push({ id: nextId("b"), projectId: p.id, name: "Board", sortIndex: 1000 })
      save(db)
      return p
    },
    async createColumns(projectId: Id, names: string[]): Promise<Column[]> {
      const db = load()
      const board = db.boards.find((b)=>b.projectId===projectId) || { id: nextId("b"), projectId, name:"Board", sortIndex: 1000 }
      if (!db.boards.find((b)=>b.id===board.id)) db.boards.push(board)
      const start = db.columns.filter((c)=>c.boardId===board.id).length
      const cols: Column[] = names.map((n,i)=> ({ id: nextId("c"), boardId: board.id, name: n, sortIndex: (start+i+1)*1000 }))
...
function ensureDemoTasks(db: any, projectId: Id, targetCount: number) {
  const existing = db.tasks.filter((t: Task) => t.projectId === projectId).length
  ...
  if (cols.length === 0) {
    cols = ["To Do","Doing","Done"].map((name, i) => ({ id: nextId("c"), boardId: board.id, name, sortIndex: (i+1)*1000 }))
    db.columns.push(...cols)
  }
  // 既存の siblings max sortIndex per column
  const byCol = new Map<string, number>()
  for (const c of cols) {
    const max = db.tasks.filter((t: Task)=>t.columnId===c.id).reduce((m: number, t: Task)=>Math.max(m, t.sortIndex), 0)
    byCol.set(c.id, max)
  }
  const today = new Date(); today.setHours(0,0,0,0)
  const namePool = [
    "見積作成", "要件整理", "初回ヒアリング", "契約手続き", "請求処理", "導入準備", "QA対応", "改善提案",
    "仕様レビュー", "デモ実施", "社内承認", "納期調整", "検収対応", "フォローアップ", "資料作成"
  ]
  for (let i = 0; i < need; i++) {
    ...
    const sd = new Date(today); sd.setDate(sd.getDate()+offsetStart)
    const dd = new Date(today); dd.setDate(dd.getDate()+offsetDue)
    const title = `デモ案件 ${existing + i + 1} - ${namePool[i % namePool.length]}`
    const task: Task = {
      id: nextId("t"),
      projectId,
      columnId: col.id,
