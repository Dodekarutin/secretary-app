...
- カンバン: 列追加予定に先立ち、表示・カード D&D（列間/列内並び替え）、列ヘッダーの D&D/矢印キー並び替え、タスク詳細ドロワー
  - **タスク完了機能**: カード上のチェックボタンでタスクを完了/未完了に切り替え（完了したタスクは緑色で表示）
...
- **Undo/Redo機能**: 操作を間違えた場合に元に戻せる（Ctrl+Z / Ctrl+Y、またはヘッダーのボタンで操作）
...
# タスク分解ページでGemini APIを使ったAI分解を利用する場合は以下を設定してください。
# 未設定時はローカル分解にフォールバックします。
VITE_GEMINI_API_KEY=あなたのGemini APIキー

# 音声入力機能を使用する場合（OpenAI Whisper API）
# タスク分解ページで音声入力を使う場合は以下を設定してください。
# 未設定時は音声入力ボタンが無効になります。
...
- APIキーは `.env` ファイルに設定してください（設定画面からは設定できません）
- `.env` ファイルは `.gitignore` に登録済みのため、リポジトリにはコミットされません
- **公開環境では、APIキーをブラウザに直接露出させないよう、必ずサーバープロキシ経由でAPIを呼び出すことを推奨します**
- APIキーは絶対に公開せず、安全な場所に保管してください
...
2. （オプショナル）AI 分解機能や音声入力を使う場合は `.env` ファイルを作成して API キーを設定します。
   ...
   - **重要：APIキーは設定画面からは設定できません。`.env` ファイルに設定してください**
   - **公開環境では、APIキーをブラウザに直接露出させないよう、必ずサーバープロキシ経由でAPIを呼び出すことを推奨します**
